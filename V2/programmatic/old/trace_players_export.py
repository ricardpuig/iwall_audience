...
for m in container_ids:
	...
	for n in data["field_report"]:
		...
		if fr: 
			...
			if player_id:
				...
				player_field_report['codigo_soporte']="TRACE-"+str(player_id)
			try: 
			...
			if (str(player_field_report['player_id']) in player_id_blacklist)or (str(player_field_report['player_container_id']) in player_container_blacklist)or (str(player_field_report['display_unit_container_id']) in du_container_blacklist) or (player_field_report['player_id'] in temporary_players):
				...
				if player_field_report['display_unit_container_id']!="": 
					sql_select = "SELECT id, name from malls where broadsign_container_id ={id}".format(id=player_field_report['display_unit_container_id'])
					print(sql_select)
					mycursor.execute(sql_select)
					records= mycursor.fetchall()
					if len(records)==0:
					...
					for row in records:  
						...
						player_field_report['mall_name']=row[1]
	...
	df_report_trace= df_field_report[['player_id', 'player_screens', 'codigo_soporte', 'screen_resolution', 'geolocation', 'latitude', 'longitude', 'address', 'zipcode', 'container_name' , 'country', 'mall_name' ]]
	df_report_trace['tipo_soporte']="MUPI"
	df_report_trace['tipo_emission']="DIGITAL"
	df_report_trace['ubicacion']="INDOOR"
	df_report_trace['ID_digital']= df_report_trace['player_id']    
	df_report_trace['cp']= df_report_trace['zipcode']    
    
	df_report_trace.to_csv('Trace/trace_players.csv',index=False)
