...
report_template= "YA67AB0ZMYMQW5JG28C7977HH4F4"
alarm_template  = "YA67AB0ZMYMQW5JG28C7977HH4F4"
...
def alarms_check(player_field_report):
	...
	if (str(player_field_report['player_id']) in player_id_blacklist)or (str(player_field_report['player_container_id']) in player_container_blacklist)or (str(player_field_report['du_container_id']) in du_container_blacklist) or (player_field_report['player_id'] in wifi_players) or (player_field_report['player_id'] in temporary_players):
		...
		main_player_status.append("\n    Player:   " + player_field_report['player_name'] + " \n Contents playing: " + player_field_report['contents'])
...
def daily_report(report_type):
	...
	if (len(missing_players)==0) and (len(offline_players)==0): 
		message1= " All players reporting OK"

	else: 
		message1= "Players sin conexión: \n"
	...
	if len(black_players)>0:
		message1: message1 + "\n" + "Warning, players with no content or suspected black screen: "
	...
	resp = client.send_message(
				message={
					"to": {
						"email": "mariajesus@ledsmarketing.com",
					},
...
#select only LA VILLA PLAYER FOLDER
				...
				try:
						...
						for m in data4["host"]:
							...
							for n in data5["content"]:
								content_names=content_names + " \b || \b "+ n['name']
				except: 
