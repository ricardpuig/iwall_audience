...
if option == 'all':
    ...
    print("Resetting mall model....")
    all_campaigns=False
    reset_mall = sys.argv[2]
    if sys.argv[2] == "all":
    ...
    print("reset_mall_model: will restablish the mall model . additional1=all will restablish all malls, else the container id ")
    print("list_malls")
    print("mall_check")
    print("find_campaign")
    exit()
...
for row in campaigns:  #for each campaign to analyze
  ...
  for n in data["campaign_performance"]:
    ...
    if n["total"]>100:
        ...
        val= (country, campaign_name, n["total_impressions"],str(n["played_on"]),DAILY_REPETITIONS,n["reservable_id"])
        mycursor.execute(sql,val)
        mydb.commit()
  ...
  for n in data["display_unit_performance"]:
     ...
     campaign_display_unit_performance["repetitions"]= campaign_days_total * DAILY_REPETITIONS 
     ...
     try: 
        ...
        for m in data_name["display_unit"]:
          ...
          campaign_display_unit_performance["container_id"]=sys.argv[4]
          campaign_display_unit_performance["screen_count"]=EXCLUSIVE_SCREENS
          num_screens_total = EXCLUSIVE_SCREENS
          ...
          for o in data_mall_name["container"]:
              ...
              print("Repetitions for site "+ str(o["name"])+ " and Display unit " + str(m["name"]) + ": " + str(DAILY_REPETITIONS)* campaign_days_total + " ( " + str(EXCLUSIVE_SCREENS) +" screens)" )

        campaign_display_unit_performance["reservation_id"]=n["reservable_id"]
        campaign_display_unit_performance["repetitions"]=DAILY_REPETITIONS

        num_plays_total = DAILY_REPETITIONS * campaign_days_total
        ...
        val= (campaign_name, n["display_unit_id"],sys.argv[4],EXCLUSIVE_SCREENS,str(m["name"]),o["name"].encode('utf-8', errors ='ignore'),n["total_impressions"],DAILY_REPETITIONS,n["reservable_id"])
        mycursor.execute(sql,val)
        mydb.commit()
  ...
  for row_0 in records_0:  #for each display unit
      ...
      du_bs_screens=EXCLUSIVE_SCREENS
      du_bs_repetitions=campaign_days_total*DAILY_REPETITIONS
      ...
      exclusive_screen_container_id= sys.argv[4]

      #Extracting mall id and number of screens for malls table
      print("Getting information from malls database:")
      sql_select_getmall = "SELECT id, screens, default_screen_day_impressions, default_screen_day_views, name, dwell_time FROM malls WHERE broadsign_container_id='%s'" % (str(exclusive_screen_container_id))
