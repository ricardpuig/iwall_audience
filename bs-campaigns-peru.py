...
print("campaigns to analyze: ", campaigns_to_analyze)

if campaigns_to_analyze == 'all':
  all_campaigns= True
else:
  all_campaigns=False
...
for row in campaigns:  #for each campaign to analyze
  ...
  for n in data["campaign_performance"]:
    ...
    if n["total"]>100:
        ...
        val= ("PERU", campaign_name, n["total_impressions"],str(n["played_on"]),n["total"],n["reservable_id"])
        mycursor.execute(sql,val)
        mydb.commit()
  ...
  for row_0 in records_0:  #for each display unit
      ...
      if du_bs_repetitions == 0:
        ...
        print("No repetitions from broadsign... aproximating")
      ...
      for day in campaign_days:
          ...
          try: 
            ...
            print("repetitions - "+ str(du_bs_repetitions) + " screens - " +str(du_bs_screens))
 
         
            #numero de impresiones para pantallas del display unit
            du_day_repetitions=du_bs_repetitions/total_days  #numero de repeticiones para ese dia, dividimos total entre numero de dias de la campana
            #calculo del factor corrector de impresiones en base a las repeticiones de ese dia
            screen_day_reps = du_day_repetitions/du_bs_screens
      ...
      try:         
        ...
        val= ("PERU", campaign_name,row_0[3],row_0[0],row_0[2], str(row_0[4].encode('utf-8', errors ='ignore')),str(row_0[5].encode('utf-8', errors ='ignore')),f_child,f_young,f_adult,f_senior,0,m_child,m_young,m_adult,m_senior,0,0,0,0,0,0,reservation_id)
        mycursor.execute(sql,val)
        mydb.commit()
      except: 
